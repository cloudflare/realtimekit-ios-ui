...
open class RtkMoreButtonControlBar: RtkControlBarButton {
    ...
    private func getMenu() -> [MenuType] {
        ...
        if meeting.localUser.permissions.miscellaneous.transcriptionEnabled {
            menus.append(.aiTranscript)
        }

        let cmPerms = meeting.localUser.permissions.connectedMeetings
        if cmPerms.canAlterConnectedMeetings ||
            ((cmPerms.canSwitchConnectedMeetings || cmPerms.canSwitchToParentMeeting) && meeting.connectedMeetings.isActive)
        {
            menus.append(.breakoutRooms(notificationMessage: ""))
        }
