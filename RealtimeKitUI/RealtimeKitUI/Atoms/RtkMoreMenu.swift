...
public enum MenuType {
    ...
    case images
    case aiTranscript
    case breakoutRooms(notificationMessage: String)
    case cancel
}
...
extension MenuType {
    func getAccessIdentifier() -> String {
        ...
        case .aiTranscript:
            "MoreMenu_Option_AiTranscript"

        case .breakoutRooms:
            "MoreMenu_Option_BreakoutRooms"
...
public class RtkMoreMenu: UIView {
    ...
    private func getModel(features: [MenuType]) -> [BottomSheetModel] {
        ...
        for feature in features {
            ...
            case .aiTranscript:
                model.append(BottomSheetModel(type: feature, image: RtkImage(image: ImageProvider.image(named: "icon_transcript")), title: "Transcripts", onTap: { [weak self] _ in
                    guard let self else { return }
                    onSelect(feature)
                    hideSheet()
                }))
            case let .breakoutRooms(notificationMessage):
                // Use the dedicated breakout rooms asset when available (drop a PDF into
                // icon_breakout_rooms.imageset to activate it); fall back to the SF Symbol
                // square.grid.2x2 (2×2 grid, available iOS 14+), then icon_participants.
                let breakoutIcon = ImageProvider.image(named: "icon_breakout_rooms")
                    ?? UIImage(systemName: "square.grid.2x2")
                    ?? ImageProvider.image(named: "icon_participants")
                model.append(BottomSheetModel(unreadCount: notificationMessage, type: feature, image: RtkImage(image: breakoutIcon), title: "Breakout Rooms", onTap: { [weak self] _ in
                    guard let self else { return }
                    onSelect(feature)
                    hideSheet()
                }))
