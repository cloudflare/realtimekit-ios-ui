...
public class RtkChatViewController: RtkBaseViewController, NSTextStorageDelegate, SetTopbar {
    ...
    private func setupViews() {
        ...
        messageTableView.addGestureRecognizer(longPress)
    ...
    @objc private func handleLongPress(_ gesture: UILongPressGestureRecognizer) {
        ...
        let point = gesture.location(in: messageTableView)
        guard let indexPath = messageTableView.indexPathForRow(at: point),
              let messages,
              indexPath.row < messages.count else { return }
        let message = messages[indexPath.row]
        showChatMessageActionsSheet(message: message)
    }

    private func showChatMessageActionsSheet(message: ChatMessage) {
        let sheet = UIAlertController(title: nil, message: nil, preferredStyle: .actionSheet)
        let isPrivateMessage = (message.targetUserIds?.isEmpty == false)
        let isOwnMessage = message.userId == meeting.localUser.userId

        // Pin/Unpin: matches web SDK — no canPinMessage permission check, shown for all non-private messages.
        if !isPrivateMessage {
            let pinTitle = message.pinned ? "Unpin" : "Pin"
            sheet.addAction(UIAlertAction(title: pinTitle, style: .default) { [weak self] _ in
                guard let self else { return }
                if message.pinned {
                    meeting.chat.unpin(messageId: message.id) { [weak self] error in
                        if error != nil {
                            self?.showToast("Failed to unpin message")
                        }
                    }
                } else {
                    meeting.chat.pin(messageId: message.id) { [weak self] error in
                        if error != nil {
                            self?.showToast("Failed to pin message")
                        }
                    }
                }
            })
        ...
        if isOwnMessage, message.type == .text {
            sheet.addAction(UIAlertAction(title: "Edit", style: .default) { [weak self] _ in
                self?.showEditDialog(message: message)
            })
        }
        ...
        if sheet.actions.count == 1 { return } // only Cancel, nothing to show
        present(sheet, animated: true)
    }

    private func showEditDialog(message: ChatMessage) {
        let currentText = (message as? TextMessage)?.message ?? ""
        ...
        alert.addAction(UIAlertAction(title: "Save", style: .default) { [weak self] _ in
            ...
            meeting.chat.editTextMessage(messageId: message.id, newMessage: newText) { [weak self] error in
                if error != nil {
                    self?.showToast("Failed to edit message")
                }
            }
        })
    ...
    private func showDeleteConfirmDialog(message: ChatMessage) {
        let alert = UIAlertController(title: "Delete message",
                                      message: "Are you sure you want to delete this message?",
                                      preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: "Delete", style: .destructive) { [weak self] _ in
            guard let self else { return }
            meeting.chat.deleteMessage(messageId: message.id) { [weak self] error in
                if error != nil {
                    self?.showToast("Failed to delete message")
                }
            }
        })
        alert.addAction(UIAlertAction(title: "Cancel", style: .cancel))
    ...
    private func showToast(_ text: String) {
        DispatchQueue.main.async { [weak self] in
            self?.view.showToast(toastMessage: text, duration: 2.0, uiBlocker: false)
        }
    }
