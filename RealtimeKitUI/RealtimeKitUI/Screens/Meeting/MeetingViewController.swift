...
public class MeetingViewController: RtkBaseMeetingViewController {
    ...
    private weak var breakoutOverlay: UIView?
...
extension MeetingViewController: MeetingViewModelDelegate {
    ...
    func breakoutRoomChanging(destinationMeetingId: String) {
        // Remove any existing overlay before adding a new one — guards against double-fire from the SDK.
        breakoutOverlay?.removeFromSuperview()
        // Show a full-screen loading overlay while the SDK switches the local participant to a breakout room.
        // Dismissed automatically when the meeting is re-initialised via breakoutRoomChanged(error:).
        let overlay = UIView()
        overlay.backgroundColor = view.backgroundColor
        breakoutOverlay = overlay
        overlay.translatesAutoresizingMaskIntoConstraints = false
        let spinner = UIActivityIndicatorView(style: .large)
        spinner.color = DesignLibrary.shared.color.textColor.onBackground.shade900
        spinner.translatesAutoresizingMaskIntoConstraints = false
        spinner.startAnimating()
        let label = UILabel()
        let isReturningToMain = destinationMeetingId == meeting.connectedMeetings.parentMeeting?.id
        label.text = isReturningToMain ? "Returning to Main Room..." : "Joining breakout room..."
        label.font = UIFont.systemFont(ofSize: 14)
        label.textColor = DesignLibrary.shared.color.textColor.onBackground.shade700
    ...
    func breakoutRoomChanged(error: MeetingError?) {
        ...
        if let error {
            ...
            // On success the SDK re-uses the same RealtimeKitClient instance.
            // MeetingViewModel.onMeetingChanged re-registers the VM's own listeners
            // (self, participants, plugins, polls, stage). Re-register the chat listener
            // owned by MeetingViewController here, since it is cleared by the SDK during
            // the internal leave/re-init cycle just like the others.
            meeting.addChatEventListener(chatEventListener: self)
            viewModel.delegate?.refreshMeetingGrid()
            // Update topbar title to reflect the current room after the switch
            let selfId = meeting.localUser.userId
            let currentRoom = meeting.connectedMeetings.meetings
                .first { $0.participants.contains { $0.id == selfId || $0.customParticipantId == selfId } }
            let newTitle = currentRoom?.title
                ?? meeting.connectedMeetings.parentMeeting?.title
                ?? meeting.meta.meetingTitle
            topBar.updateMeetingTitle(newTitle)
        }
    }
    ...
    private func showWaitingRoomIfNeeded(status: ParticipantMeetingStatus, onComplete: @escaping () -> Void) {
...
extension MeetingViewController: RtkChatEventListener {
    public func onMessageEdited(message _: ChatMessage) {}

    public func onMessageDeleted(messageId _: String) {}
