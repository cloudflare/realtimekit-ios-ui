...
protocol MeetingViewModelDelegate: AnyObject {
    ...
    /// Called when the SDK begins switching the local participant to a breakout room.
    func breakoutRoomChanging(destinationMeetingId: String)
    /// Called after the SDK finishes a room switch. nil = success; non-nil = failed.
...
extension MeetingViewModelDelegate {
    ...
    func breakoutRoomChanging(destinationMeetingId: String) {}
    func breakoutRoomChanged(error _: MeetingError?) {}
}
...
public final class MeetingViewModel {
    ...
    private(set) var connectedMeetingsListener: RtkConnectedMeetingsListener?
    ...
    private func setupConnectedMeetingsListener() {
        connectedMeetingsListener = RtkConnectedMeetingsListener(rtkClient: rtkClient)

        connectedMeetingsListener?.onChangingMeeting = { [weak self] meetingId in
            // Notify VC to show a loading overlay while the SDK switches rooms
            self?.delegate?.breakoutRoomChanging(destinationMeetingId: meetingId)
        }

        connectedMeetingsListener?.onMeetingChanged = { [weak self] error in
            guard let self else { return }
            if error == nil {
                // Room switch succeeded — re-register the VM's own feature listeners;
                // they are cleared during the internal leave/re-init cycle.
                // NOTE: MeetingViewController re-registers its own chat listener in
                // breakoutRoomChanged(error:) on success. Any other component that
                // registered feature listeners directly must do the same.
                rtkClient.addSelfEventListener(selfEventListener: self)
                rtkClient.addParticipantsEventListener(participantsEventListener: self)
                rtkClient.addPluginsEventListener(pluginsEventListener: self)
                rtkClient.addPollsEventListener(pollsEventListener: self)
            ...
            delegate?.breakoutRoomChanged(error: error)
        }
    }
